...
End-to-end example demonstrating how to use the LemonSlice API with LiveKit. Use this when you want full control over your video agent pipeline (for example, with your own STT, LLM, and TTS components) while LemonSlice handles avatar generation. Orchestration, infrastructure, and UI remain in your stack, with the LiveKit agent built using the [LiveKit Python SDK](https://github.com/livekit/agents).
